...
if __name__ == '__main__':
    with torch.no_grad():     
        ...
        dcrnet.load_state_dict(torch.load('./L2Loss_LS.pth'))
        dcrnet.to(device)
        dcrnet.eval()
        ...
        for idx in range(FileNo, FileNo + No_subs):
            ...
            path = "rec_Input_" + str(FileNo) + '_real.mat'
            scio.savemat(path, {'recons_r':recons_r})

            path = "rec_Input_" + str(FileNo) + '_imag.mat'
            scio.savemat(path, {'recons_i':recons_i})
            print('Reconstruction Ends, then go to matlab code Folder for postprocessing')